...
class ModelCatalogGift extends Model {
	...
	public function getProductGifts($data = array()) {
		$sql = "SELECT DISTINCT mg.product_id, mg.gift_id, (SELECT AVG(rating) FROM " . DB_PREFIX . "review r1 WHERE r1.product_id = mg.product_id AND r1.status = '1' GROUP BY r1.product_id) AS rating FROM " . DB_PREFIX . "mm_gift mg LEFT JOIN " . DB_PREFIX . "product p ON (mg.product_id = p.product_id) LEFT JOIN " . DB_PREFIX . "product_description pd ON (p.product_id = pd.product_id) LEFT JOIN " . DB_PREFIX . "product_to_store p2s ON (p.product_id = p2s.product_id) WHERE p.status = '1' AND p.date_available <= NOW() AND p2s.store_id = '" . (int)$this->config->get('config_store_id') . "' GROUP BY mg.product_id";
